...
export default function DirecaoPanel({ answers, setSel, setNota, notes, setNotes, onAdvance, ctx }) {
  const a = answers;

  const handleGenerate = () => {
    const text = phasePrompt(2, ctx);
    navigator.clipboard?.writeText(text);
    alert('Prompt copiado!\n\nCole no Claude ou em qualquer IA para gerar o relatório de direção de arte.');
  };
