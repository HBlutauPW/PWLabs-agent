import { SCENE_TYPES, SCENE_OPTIONS, CAMERA_FNS } from '../data';
...
function AtoTag({ ato }) {
  const t = SCENE_TYPES[ato];
  const c = COLOR_MAP[t?.color] || COLOR_MAP.purple;
  return (
    <span
      className={styles.atoTag}
  ...
  return (
    <div className={styles.card}>
      <div className={styles.cardHead} style={{ borderLeftColor: c.text }}>
        <div className={styles.cardNum} style={{ background: c.bg, color: c.text }}>
          {index + 1}
        </div>
        <div className={styles.cardMeta}>
          <select
            className={styles.atoSelect}
            value={scene.ato}
      ...
      <div className={styles.cardBody}>
        ...
        <div className={styles.fieldFull}>
          ...
          <textarea
            value={scene.locucao}
            onChange={e => update('locucao', e.target.value)}
            placeholder="Texto sugerido para esta cena..."
            rows={2}
          />
        </div>
        ...
        <div className={styles.field}>
          ...
          <input
            type="text"
            value={scene.cam}
            onChange={e => update('cam', e.target.value)}
...
export default function StoryboardPanel({ story, setStory, notes, setNotes, onAdvance, ctx }) {
  const handleChange = (i, updated) => {
    setStory(prev => prev.map((s, idx) => idx === i ? updated : s));
  };

  const handleRemove = (i) => {
    if (story.length > 1) setStory(prev => prev.filter((_, idx) => idx !== i));
  };

  const handleAdd = () => {
    setStory(prev => [...prev, { ato: 'E', cena: '', funcao: '', cam: '', locucao: '', duracao: '' }]);
  };
    ...
    <div>
      ...
      <div className={styles.structureRow}>
        ...
        {Object.entries(SCENE_TYPES).map(([k, v]) => {
          ...
          return (
            <span key={k} className={styles.structTag} style={{ background: c.bg, color: c.text, borderColor: c.border }}>
              {v.label}
            </span>
          );
        })}
